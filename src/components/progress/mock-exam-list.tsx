...
export function MockExamList({ exams }: { exams: MockExam[] }) {
  if (exams.length === 0) {
    return <p className="text-sm text-slate-500">Henüz deneme eklenmedi.</p>;
  }

  return (
    <div className="space-y-2">
      {[...exams].reverse().map((exam) => (
        <details key={exam.id} className="group rounded-lg border border-slate-200">
          ...
          <div className="overflow-x-auto border-t border-slate-100 px-4 pb-3 pt-2">
            {exam.subject_nets.length === 0 ? (
              <p className="py-2 text-sm text-slate-500">Ders bazlı detay kaydedilmemiş.</p>
            ) : (
                ...
                <tbody className="divide-y divide-slate-50">
                  {exam.subject_nets.map((s) => (
                    <tr key={s.subject_name}>
                      <td className="py-2 pr-4 font-medium text-slate-900">{s.subject_name}</td>
                      <td className="py-2 pr-4 text-emerald-700">{s.correct}</td>
                      <td className="py-2 pr-4 text-red-700">{s.wrong}</td>
                      <td className="py-2 text-slate-700">{s.net}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
